# 90-days Personal DevOps Learning Plan

## My understanding of DevOps & Cloud Engineerin
- DevOps is a culture or mindset where development team collaborate with operation team to reduce time to market by adopting automation, cloud, infrastructure & scaling.
(OR) 
DevOps is a culture that is adopted by every organization to increase efficiency & accelerate SDLC.

- Cloud engineering is a field where we use servers & services over the internet without managaing hardware, which makes system faster and scalable.

## Reason for starting learning Devops & Cloud
I am learning DevOps and Cloud for better career growth and a competitive salary package. I want to transition from support role to core or engineering focused role.

## Where I want to reach
Independently I aim to:
 - Design and manage CI/CD pipelines.
 - Deploy a production-grade application on Kubernetes.
 - Troubleshoot production issues.
 - Cloud infrastructure automation.
> Long-term goal is to secure a DevOps role in a product-based company.

## Consistency
- I will focus on consistency over perfection. I will first target a 7-day streak, then 15 days, and gradually build this into a long-term habit.
- Baby steps at a time. Even on busy days, I will target at least 30 minutes learning or revising to keep the momentum.

## Current level
I am a working professional with 4-5 years experience in SRE role, with hands-on exposure to tools like GitLab, Nexus, SonarQube, and UBSD (in-house tool).

## 3 clear goals for 90 days
- Build strong fundamentals in Linux, networking, and troubleshooting.
- Gain practical experience with cloud and automation.
- Deploy a complete microservices application using Docker, Kubernetes, and CI/CD.

## 3 core DevOps skills I want to build
- Linux troubleshooting and shell scripting
- CI/CD pipeline design and debugging
- Kubernetes deployment and debugging
- CloudOps and Infrastructure creation

## Weekly time allocation
- Weekdays: ~2 hours/day
- Weekends: 4-5 hours/day
- Busy days: Minimum 30 minutes/day